...
class InventarioVendedor(tk.Frame):
    def __init__(self, parent):
        ...
        entrada_busqueda.insert(0, "Buscar en el inventario")

        def on_entry_click(event):
            if entrada_busqueda.get() == "Buscar en el inventario":
        ...
        inventario = [
            ("1", "Cuaderno", "20", "S/. 5"),
            ("2", "Lapicero", "50", "S/. 1"),
            ("3", "Plumones", "15", "S/. 4.50"),
        ]
        for producto in inventario:
            tabla.insert("", "end", values=producto)

        # Botón de exportación (opcional)
        boton_exportar = tk.Button(self, text="Exportar inventario", bg="green", fg="white", font=("Arial", 12), padx=10, pady=5)
